...
export default function ScienceLabBoard({ age, page, onComplete, onSelectLab }: { age: number; page: number; onComplete: () => void; onSelectLab: (page: number) => void }) {
  ...
  return (
    <div className="creative-board science-lab-board">
      ...
      <section className="lab-question"><span>1 · ASK</span><h4>{lab.question}</h4><p>Scientists begin with a question, not an answer.</p></section>
      <div className="lab-grid">
        ...
        <section>
          <article className="lab-card steps-card"><span>3 · TEST SAFELY</span><h4>Check each step</h4>{lab.steps.map((step, index) => <label key={step}><input type="checkbox" checked={checked.includes(index)} onChange={() => setChecked((current) => current.includes(index) ? current.filter((item) => item !== index) : [...current, index])} /><i>{index + 1}</i><p>{step}</p></label>)}</article>
          <article className="lab-card observe-card"><span>4 · OBSERVE</span><p>{lab.observation}</p></article>
        </section>
      </div>
      <section className="lab-explain">
        <div><span>5 · EXPLAIN</span><h4>Compare the evidence with your prediction</h4><p>{revealed ? lab.explanation : "Finish the safe steps, then reveal the science. A result that surprises you is still useful evidence."}</p></div>
