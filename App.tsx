...
import { isNewToChild, pickUnseenPage } from "./novelty";
import {
  ageWorldFor,
  completedCount,
  emptyProgress,
...
function ColoringBoard({ page, age, onComplete }: { page: number; age: number; onComplete: () => void }) {
  ...
  return (
    <div className="creative-board color-board">
      <div className="tool-row">
        ...
        <div className="color-tools">
          {COLORS.slice(0, 7).map((swatch) => (
            <button
              key={swatch}
              aria-label={`Choose ${colorName(swatch)}`}
            ...
            >
              <span className="swatch-check" aria-hidden="true">✓</span>
            </button>
          ))}
        </div>
      ...
      {celebrated && (
        age <= 1 ? (
          /* Finishing something is the biggest emotional payoff for the
             younger worlds (ages 1-6) — make it a Fifi character moment, not
             a static line of text. */
          <div className="success-celebrate" role="status">
            <span className="success-sparkles" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></span>
            <img src={`${import.meta.env.BASE_URL}mascot/fifi-color-spark.png`} alt="" aria-hidden="true" />
            <p>You did it! Every part is colored. ⭐</p>
          </div>
        ) : (
          <div className="success-toast" role="status">Beautiful work! You colored every part. ⭐</div>
        )
      )}
    </div>
  );
...
function Studio({
  ...
  const [fifiTipSeen, setFifiTipSeen] = useState(true);
  ...
  // Fifi's per-activity tip auto-opens only for the younger worlds (ages 1-6).
  // For 7-12, a constantly self-opening mascot reads as babyish, so the tip
  // becomes opt-in: an "Ask Fifi" button, badge-lit until the child has seen it.
  const fifiAutoOpens = age <= 1;
  useEffect(() => {
    ...
    setFifiTipSeen(seen);
    setFifiTipOpen(!seen && fifiAutoOpens);
  }, [profile.id, activity, fifiAutoOpens]);
        ...
        <section className="studio-content">
          <div className="studio-heading">
            ...
            <div className="page-picker">
              ...
              {age === 0 ? (
                /* Youngest world (ages 1-3): arrows only. A toddler cannot type
                   into a number field, and typing is not how this age should
                   move through content. */
                <span className="page-count" aria-live="polite">{unit} {page} of {total}</span>
              ) : (
              ...
              <span className={`completion-marker ${completedHere ? "done" : ""}`}>{completedHere ? "✓ Done" : isNewToChild(page, profileProgress.activities[activity].completed, age) ? "✨ New to you" : "In progress"}</span>
              <button onClick={guard(() => onPage(page + 1))} aria-label={`Next ${unit.toLowerCase()}`} disabled={page === total}>→</button>
            </div>
            {total > 1 && (
              <button
                className="surprise-button"
                onClick={guard(() => onPage(pickUnseenPage(total, profileProgress.activities[activity].completed, age, page, Date.now())))}
                aria-label={`Surprise me — jump to a ${unit.toLowerCase()} I haven't tried`}
              >✨ Surprise me</button>
            )}
            {!fifiAutoOpens && (
              <button
                className={`ask-fifi-button ${!fifiTipSeen ? "has-new" : ""}`}
                onClick={() => setFifiTipOpen(true)}
                aria-label={!fifiTipSeen ? "Ask Fifi — new tip" : "Ask Fifi for a tip"}
              >
                <img src={fifiMascot} alt="" aria-hidden="true" />
                <span>Ask Fifi</span>
              </button>
            )}
            </div>
